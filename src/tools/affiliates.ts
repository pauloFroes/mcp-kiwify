...
export function registerAffiliateTools(server: McpServer) {
  server.registerTool(
    "list_affiliates",
    {
      ...
      description:
        "List all affiliates. Filter by status, product, or search by name/email. Returns affiliate name, email, product, commission (in cents), and status.",
      inputSchema: {
    ...
    async (args) => {
      try {
        ...
        return toolError(`Failed to list affiliates: ${(error as Error).message}`);
      }
    },
  );
  ...
  server.registerTool(
    ...
    async ({ affiliate_id }) => {
      try {
        ...
        return toolError(`Failed to get affiliate: ${(error as Error).message}`);
      }
    },
  );
  ...
  server.registerTool(
    "edit_affiliate",
    {
      ...
      description:
        "Update an affiliate's commission or status. Commission is in cents (e.g. 4600 = R$46.00). Status can be: active, blocked, or refused.",
      inputSchema: {
    ...
    async ({ affiliate_id, commission, status }) => {
      try {
        ...
        return toolError(`Failed to edit affiliate: ${(error as Error).message}`);
      }
    },
  );
}
