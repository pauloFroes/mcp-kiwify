...
export function registerFinanceTools(server: McpServer) {
  server.registerTool(
    "get_balance",
    {
      ...
      description:
        "Get the current account balance. Returns available and pending amounts in cents (e.g. 236961 = R$2,369.61).",
      inputSchema: {},
      annotations: {
        readOnlyHint: true,
  ...
  server.registerTool(
    ...
    async ({ legal_entity_id, page_size, page_number }) => {
      try {
        ...
        return toolError(`Failed to list payouts: ${(error as Error).message}`);
      }
    },
  );
  ...
  server.registerTool(
    "create_payout",
    {
      title: "Create Payout",
      description:
        "Request a withdrawal (payout). Amount is in cents (e.g. 5000 = R$50.00). This initiates a real financial transfer.",
      inputSchema: {
    ...
    async ({ amount }) => {
      try {
        ...
        return toolError(`Failed to create payout: ${(error as Error).message}`);
      }
    },
  );
}
