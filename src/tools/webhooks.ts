...
export function registerWebhookTools(server: McpServer) {
  server.registerTool(
    "list_webhooks",
    {
      title: "List Webhooks",
      description:
        "List all configured webhooks. Returns webhook name, URL, triggers, target products, and verification token.",
      inputSchema: {
    ...
    async (args) => {
      try {
        ...
        return toolError(`Failed to list webhooks: ${(error as Error).message}`);
      }
    },
  );
  ...
  server.registerTool(
    "create_webhook",
    {
      ...
      description:
        'Create a new webhook to receive event notifications. Triggers: boleto_gerado, pix_gerado, carrinho_abandonado, compra_recusada, compra_aprovada, compra_reembolsada, chargeback, subscription_canceled, subscription_late, subscription_renewed. Set products to "all" or a specific product UUID.',
      inputSchema: {
    ...
    async ({ name, url, products, triggers, token }) => {
      try {
        const body: Record<string, unknown> = { name, url, products, triggers };
        if (token) body.token = token;

        const data = await apiRequest("/webhooks", "POST", body);
        return toolResult(data);
      } catch (error) {
        return toolError(`Failed to create webhook: ${(error as Error).message}`);
      }
    },
  );
  ...
  server.registerTool(
    ...
    async ({ webhook_id }) => {
      try {
        ...
        return toolError(`Failed to delete webhook: ${(error as Error).message}`);
      }
    },
  );
}
