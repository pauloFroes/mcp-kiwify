...
export function registerEventTools(server: McpServer) {
  server.registerTool(
    "list_event_participants",
    {
      ...
      description:
        "List participants of an event product. Returns ticket summary (max, available, issued, sold, checkins) and participant details. Filter by check-in status, date ranges, batch, phone, CPF, or order.",
      inputSchema: {
    ...
    async (args) => {
      try {
        const data = await apiRequest(
          `/events/${args.product_id}/participants`,
          "GET",
          undefined,
          {
        ...
        return toolError(`Failed to list event participants: ${(error as Error).message}`);
      }
    },
  );
}
