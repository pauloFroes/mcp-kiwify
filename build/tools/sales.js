...
export function registerSalesTools(server) {
    server.registerTool("list_sales", {
        title: "List Sales",
        description: "List sales within a date range (max 90 days). Filter by status, payment method, product, or affiliate. Use view_full_sale_details for payment breakdown and tracking info.",
        inputSchema: {
        ...
        try {
            ...
            return toolError(`Failed to list sales: ${error.message}`);
        }
    });
    server.registerTool("get_sale", {
        title: "Get Sale Details",
        description: "Get full details of a specific sale by ID. Returns customer info, payment breakdown, tracking, card details, and affiliate commissions.",
        inputSchema: {
        ...
        try {
            ...
            return toolError(`Failed to get sale: ${error.message}`);
        }
    });
    server.registerTool("refund_sale", {
        title: "Refund Sale",
        description: "Refund a sale. For PIX payments, optionally provide a PIX key for the refund. This action is irreversible.",
        inputSchema: {
        ...
        try {
            ...
            return toolError(`Failed to refund sale: ${error.message}`);
        }
    });
    server.registerTool("get_sales_stats", {
        title: "Get Sales Statistics",
        description: "Get aggregated sales statistics: approval rate, total sales count, net amount (in cents), refund rate, chargeback rate, and boleto conversion rate. Optionally filter by product and date range.",
        inputSchema: {
        ...
        try {
            ...
            return toolError(`Failed to get sales stats: ${error.message}`);
        }
    });
}
